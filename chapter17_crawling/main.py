...
requests 라이브러리 : 파이썬에서 HTTP 요청을 보내기 위해 가장 널리 사용되는
    ...
    pop install requests
        ...
        1. 웹 크롤링의 이해
            ...
            웹 크롤링(Web Crawling)이란 완성된 웹 페이지에서 필요한 정보를 수집하고 선별하여 추출하는 과정으로,
            Web Scraping 이라고 하기도 함.
            1) HTML의 개념
                hyper Text Markup Language 의 약자로 '웹 페이지를 만드는 문법을 갖춘 언어'
...
<html>
    ...
    body
        ...
        3) URL : Uniform Resource Locator 의 약자로 어떤 자원의 위치를 표기하는 방법을 의미함
            https://www.google.com 과 같은 인터넷 주소를 입력하는데, 이때 입력한 주소가 URL에 해당함
'''
import requests
...
https://search.naver.com/search.naver?where=nexearch&sm=top_sug.pre&fbm=0&acr=3&acq=python&qdt=0&ie=utf8&query=파이썬

검색하는 것이 함수 개념이라면 결과값은 return 에 해당될텐데 검색어는 argument
search("파이썬") -> return 검색 결과창

입력한 검색어 "파이썬" 은 query라는 매개변수로 전달됩니다.
search(query = "파이썬")           # keyword argument와 같은 형태라고 볼수 있습니다.

매개변수를 입력하지 않을 경우에는 default 값으로 적용되기 때문에 나머지는 지워도 무방
...
#     "io": "utf8",
# }
# response = requests.get(url,params=param)           # dictionary 인 param 을 params 속성에 대입
...
# url = "https://m.comic.naver.com/webtoon/list?"
# param = {
#     "titleId":822556,
# }
