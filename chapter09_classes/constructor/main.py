...
그렇다면 처음부터 값이 있는 인스턴스를 생성하면 코드라인이 줄어들지 않을까.
    이에 대한 해답이 생성자라는 개념이다.
    C / Java의 경우의 생성자는 클래스명과 동일한데, 또 python만 이상한 방식
->__init__()라는 메서드 
...
#
#     def display_info(self):
#         print(f"사탕의 모양은 {self.shape}이고, 색깔은 {self.color}이다.")
#
...
예제1.
    ...
    지시 사항
        ...
        2) 학생의프로필을 출력하는 인스턴스 메서드 print_profile()를 작성하시오.(call1()타입으로 : 매개변수 없음 리턴 없음)
        3) 학생의 성적을 변경할 수 있는 인스턴스 매서드 set_grade() 를 작성하시오
            -> 이부분이 고민해볼만한 요소 // set_info()매서드 검색해보십소 컨트롤 시프트 f 눌러서
        4) 세 명의 학생 인스턴스를 생성하고, 각 학생의 정보를 출력한 다음, 성적을 변경하고 다시 출력하시오.
    ...
    이후 student 1  성적을 A+ 2는 A 3은 B+로 성적을 변경하고
...
#         self.grade = grade
#     # 객체의 인스턴스 변수 값을 출력해주는 매서드
#     def print_profile(self):
#         print(f"학생 이름 : {self.name}\n학번 : {self.student_id}\n성적 : {self.grade}")
#
#     # 이상의 코드는 콘솔에 찍히기만 하고 연산이 불가능하기에 getter은 call() 유형으로 작성하기 보다는
...
# 인스턴스 변수에 값을 대입할 때 제약을 걸기 위해 method를 경유하여 값을 대입하도록 권장함
# -> setter(call2()유형)/ getter(call3()유형) 라는 개념 -> 클래스로 돌아가 다시 매서드추가하심됨


'''
1. Setter / Getter 이란?
    1) Setter : 객체의 속성 값(인스턴스 변수)을 변경하는 메서드
    2) Getter : 객체의 속성 값(인스턴스 변수)을 조회하는 매서드
    
    3) 왜 Setter /Getter 을 사용하는가?
        (1) 데이터 보호 및 무결성 유지
            : 속성 값을 직접 변경하는 경우, 잘못된 값이 입력될 가능성이 높음.
            : Setter를 사용하면 특정 조건을 만족하는 값만 속성에 대입 가능
        (2) 객체의 캡슐화(Encappsulation) 실현
            : 객체의 내부 데이터를 외부에서 직접 수정하는 것을 방지
            : 대신 메서드를 통해서만 접근하도록 제한하여 보안성을 높임
            
        (3) 추후 유짖 보수 및 확장성 용이
            : Setter / Getter을 사용하면 특정 속성에 대한 로직을 쉽게 변경 가능(if절 바꾸는거 생각)
            : 예를 들어 특정 속성을 설정할 때 추가적인 검증이 필요하면 Setter에서 처리 가능
            
2. 이제 클래스를 정의할 때 기본적으로 Setter / Getter을 타이핑하면서 형태를 배울 예정
...
class Person:
    ...
    # set_age / set_address를 정의하는데, set_age의 경우 0 미만 및 200 초과는 입력 불가능한 로직 작성하삼
...
print(f"나이는 {person.get_age()}살인데. 만으로는 {person.get_age()-2}살입니다.") # 이런 이유로 getter 사용
